...
from servebot_bringup.msg import Speed
...
#New addon for computing quaternion
        ...
        self._motor_speeds = rospy.Subscriber('vtarget', Speed, self._Update_Both_Speeds)

    def _Update_Both_Speeds(self, data):
        self._left_wheel_speed_ = data.left
        self._right_wheel_speed_ = data.right
        # rospy.loginfo(data.left)
        # rospy.loginfo(data.right)
        speed_message = 's %.2f %.2f\r' %(float(self._left_wheel_speed_), float(self._right_wheel_speed_))
...
#######################################################################################################################
    ...
    def _WriteSerial(self, message):
        ...
        self._SerialDataGateway.Write(bytes(message, 'UTF-8'))
